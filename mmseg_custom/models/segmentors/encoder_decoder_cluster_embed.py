...
class EncoderDecoder_cluster_embed(BaseSegmentor):
    ...
    def __init__(self,
                 ...
                 desc_weights_dict_path = '/saccadenet/jfxiao/codes/ViT-Adapter/segmentation/ade_zeroshot_weights_dict.pth',
                 get_logit_mode = 'cosine_similarity',
                 sigmoid_temperature = 1.0,
                 image_embedding_normalize = True
                 ):
        super(EncoderDecoder_cluster_embed, self).__init__(init_cfg)
        ...
        if desc_weights_dict_path is None:
            test_dataset_name = test_cfg.get('test_dataset_name', None)
            self.test_dataset_name = test_dataset_name
            desc_weights_dict_path = desc_weights_paths_dict[test_dataset_name]
        ...
        self.desc_cluster2label = desc_cluster2label.cuda()
    ...
    def encode_decode(self, img, img_metas):
        ...
        out = torch.einsum('bchw,kc->bkhw', out, self.desc_clustered_embedding_bank)
        #  bx256xhw-> bx150xhw
        #options: L1 / L2 / Cosine / Dot
        #  desc_clustered_embedding_bank 384x256
        #  desc_cluster2label 150x256, multi-hot encoding
        if self.get_logit_mode == 'cosine_similarity':
            ...
            out = torch.einsum('bkhw,nk->bnhw', out, desc_cluster2label_normalized)
        elif self.get_logit_mode == 'cosine_similarity_with_sigmoid':
            out = torch.sigmoid(out / self.sigmoid_temperature)
            desc_cluster2label_normalized = F.normalize(self.desc_cluster2label, p=2, dim=1)
            out = F.normalize(out, p=2, dim=1)
            # out = torch.einsum('bkhw,nk->bnhw', out, (self.desc_cluster2label / self.desc_cluster2label.sum(dim=-1, keepdim=True)))
            out = torch.einsum('bkhw,nk->bnhw', out, desc_cluster2label_normalized)
            ...
            out = torch.sigmoid(out / self.sigmoid_temperature)
            out = torch.einsum('bkhw->bhwk', out)
            cluster_num = out.shape[-1]
            out = out.reshape(-1, cluster_num)
            diff = out.unsqueeze(1) - self.desc_cluster2label.unsqueeze(0)
            l2_dist = torch.norm(diff, dim=2) / torch.sqrt(torch.tensor(cluster_num).float())
            out = (1 - l2_dist).mean(dim=2)
    ...
    def slide_inference(self, img, img_meta, rescale):
        ...
        num_classes = self.desc_cluster2label.shape[0]
